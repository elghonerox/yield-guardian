...
import { simulationPositions } from '../fixtures/simulation';
...
export class BlockchainIntegration {
  ...
  constructor() {
    ...
    this.publicClient = createPublicClient({
      chain: config.blockchain.chainId === 11155111 ? sepolia : mainnet, // demo -> sepolia
      transport: http(config.blockchain.rpcUrl)
    });

    this.walletClient = createWalletClient({
      account: this.account,
      chain: config.blockchain.chainId === 11155111 ? sepolia : mainnet,
      transport: http(config.blockchain.rpcUrl)
    });
  }
  ...
  async getCurrentPositions(): Promise<Position[]> {
    ...
    // TODO: Implement actual position fetching from protocols (mainnet/testnet)
    return [];
  }
